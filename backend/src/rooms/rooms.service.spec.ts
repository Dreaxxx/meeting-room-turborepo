...
describe('RoomsService (unit)', () => {
  ...
  let prisma: {
    room: { findUnique: jest.Mock; findMany: jest.Mock; create: jest.Mock };
  };
  ...
  it('constructor should throw when PrismaService is missing', () => {
    // @ts-expect-error
    expect(() => new RoomsService(undefined)).toThrow(
      'PrismaService is required',
    );
  });
  ...
  it('findOne should return a room by ID', async () => {
    prisma.room.findUnique = jest
      .fn()
      .mockResolvedValue({ id: '1', name: 'Orion', capacity: 4 });
    ...
    console.log(
      '[rooms:findOne] Room:',
      JSON.stringify(prisma.room.findUnique.mock.calls[0][0]),
    );
  ...
  it('findAll should return ordered rooms', async () => {
    ...
    console.log(
      '[rooms:findAll] Rooms:',
      JSON.stringify(prisma.room.findMany.mock.calls[0][0]),
    );
